...
**Se intenta tener los comandos más comunes a la mano para ser consultados. Una especie de acordeón de comandos**  
* Las lineas sin indentación son ejemplos.  
* Las líneas indentadas que inician con `#` son la explicación de la(s) linea(s anterior(es).  
* Cada párrafo se refiere al mismo comando con varios ejemplos y/o variantes.
...
```
git add <archivo>
	# Preparpa <archivo>, es decir lo pone en el staging area.
git add -a
	# Preparpa todos los archivos que tengan cambios (incluye los que están 'untracked')
```
#### 
```
git commit
...
git rm --cached <archivo>
    # Elimina <archivo> del staging area.
```
#### 
```
...
git log <archivo>
    # Igual que git log pero sólo muestra los commits donde se incluyó o modicó <archivo>. (ideal para usar con la siguiente opción para encontrar una versión específica)
git log -p 
